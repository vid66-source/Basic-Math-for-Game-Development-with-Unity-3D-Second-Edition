...
public class EX_5_5_MyScript : MonoBehaviour {
    ...
    void Update() {
        ...
        if (!almostParallel) // two lines are not parallel
        {
            ...
            else if (d1 < 0 && da < 1 && da > 0) {
                Pd_1.transform.localPosition = P1.transform.localPosition;
                Pd_a.transform.localPosition = CalculateClosestPoint(P1.transform.localPosition,
                    Pa.transform.localPosition, Pb.transform.localPosition);
                float dist = (Pd_1.transform.localPosition - Pd_a.transform.localPosition).magnitude;
                Debug.Log("d1=" + d1 + " da=" + da + " Distance=" + dist);
            }
            else if (d1 > 1 && da < 1 && da > 0) {
                Pd_1.transform.localPosition = P2.transform.localPosition;
                Pd_a.transform.localPosition = CalculateClosestPoint(P2.transform.localPosition,
                    Pa.transform.localPosition, Pb.transform.localPosition);
                float dist = (Pd_1.transform.localPosition - Pd_a.transform.localPosition).magnitude;
                Debug.Log("d1=" + d1 + " da=" + da + " Distance=" + dist);
            }
            else if (da > 1 && d1 < 1 && d1 > 0) {
                Pd_1.transform.localPosition = CalculateClosestPoint(Pb.transform.localPosition,
                    P1.transform.localPosition, P2.transform.localPosition);
                Pd_a.transform.localPosition = Pb.transform.localPosition;
                float dist = (Pd_1.transform.localPosition - Pd_a.transform.localPosition).magnitude;
                Debug.Log("d1=" + d1 + " da=" + da + " Distance=" + dist);
            }
            else if (da < 0 && d1 < 1 && d1 > 0) {
                Pd_1.transform.localPosition = CalculateClosestPoint(Pa.transform.localPosition,
                    P1.transform.localPosition, P2.transform.localPosition);
    ...
    public Vector3 CalculateClosestPoint(Vector3 point, Vector3 a, Vector3 b) {
        Vector3 ab = b - a;
        float t = Vector3.Dot(point - a, ab)/ab.magnitude;
        t = Mathf.Clamp01(t);
        return a + ab * t;
    }
}